        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {cases.map((caseItem, index) => (
              <div
                key={caseItem.title}
                className={`fade-in fade-in-delay-${Math.min(index + 1, 3)} p-6 rounded-lg bg-card border border-border/50 card-hover`}
              >
                <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">
                  {caseItem.client}
                </p>
                <h3 className="text-lg font-medium text-foreground mb-2">{caseItem.title}</h3>
